...
function main() {
  ...
  const electionFaucetArtifactPath = path.join(
    hardhatRoot,
    "artifacts",
    "contracts",
    "ElectionFaucet.sol",
  ...
  const electionFaucetArtifact = readJson<ArtifactFile>(
    electionFaucetArtifactPath,
  );

  const trumpTokenAddress =
    deployedAddresses["ElectionFaucetModule#TrumpToken"];
  const bidenTokenAddress =
    deployedAddresses["ElectionFaucetModule#BidenToken"];
  const electionFaucetAddress =
    deployedAddresses["ElectionFaucetModule#ElectionFaucet"];

  if (!trumpTokenAddress || !bidenTokenAddress || !electionFaucetAddress) {
    throw new Error(
      [
        "Missing one or more contract addresses in deployed_addresses.json.",
        "Expected keys:",
        "  ElectionFaucetModule#TrumpToken",
        "  ElectionFaucetModule#BidenToken",
        "  ElectionFaucetModule#ElectionFaucet",
      ].join("\n"),
    );
  }

  const deploymentId = path.basename(deploymentDir);
  const outputFile = path.resolve(hardhatRoot, "../generated/contracts.ts");
  ...
  const content = generateContractsTs({
    ...
    abis: {
      ...
      electionFaucetAbi: electionFaucetArtifact.abi,
    },
    objects: {
      deploymentMeta: {
        deploymentId,
